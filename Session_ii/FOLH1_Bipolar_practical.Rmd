...
dat.scz.outcome = dat.scz_gwas %>% data.frame %>%
...
```{r mr steiger, message=F,warning=F,echo=T,eval=F}
...
```{r prepare cis pqtl data, message=F,warning=F,echo=T,eval=F}
dat.coloc.pqtl = dat.cis_pqtl_ptn %>%
...
```{r prepare bp sumstats, message=F,warning=F,echo=T,eval=F}
dat.coloc.bp = dat.scz_gwas %>%
...
```{r merge pqtl and bp data, message=F,warning=F,echo=T,eval=F}
dat.coloc.merge = left_join(dat.coloc.pqtl,dat.coloc.bp,by=c('SNP','CHR','BP')) 
```

➡️ Align alleles

```{r coloc data harmonisation, message=F,warning=F,echo=T,eval=F}
...
```r sum.flipped``` SNPs with flipped alleles

➡️ Format data for SuSie colocalisation analysis

```{r create D1 and D2 for susie colocalisation, message=F,warning=F,echo=T,eval=F}
D1 <- list(
...
➡️ Run SuSie colocalisation analysis

```{r}
res.coloc = coloc.abf(D1, D2, p1 = 1e-4, p2 = 1e-4, p12 = 1e-5)
res.coloc.info = 
data.frame(t(res.coloc$summary),n_flipped=sum.flipped,file_name=x.gene)
